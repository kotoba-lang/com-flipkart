(ns flipkart.main
  "Kotodama WASM entrypoint for the Flipkart clean-room actor (L4) — Clojure port.
  ...
  facts (`flipkart.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "flipkart")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"flipkart.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "flipkart-compat" :tier tier :entities entities} 200])
...
(defn start! [] :flipkart-compat/ready)
